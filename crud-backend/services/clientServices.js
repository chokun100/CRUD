...
export const createClients = async (clientData) => {
  const existingClient = await query("SELECT * FROM clients_tb WHERE email = $1", [clientData.email]);
  ...
  const { rows } = await query(
      ...
      [clientData.name, clientData.email, clientData.job, clientData.rate, clientData.isactive]
  );
  return rows[0];
};
